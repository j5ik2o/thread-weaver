...
object Runner extends App {
  ...
  def sendMessageToSlack(incomingWebhook: String, message: String): HttpResponse[String] = {
    Http(incomingWebhook).header("Content-type", "application/json")
...
//  }
  ...
  val incomingWebhookUrlOpt = config.getAs[String]("gatling.notice.slick.incoming-webhook-url")
  ...
  val futureTasks = if (runTaskCount > 10) {
    ...
    Future.traverse(Seq.fill(n)(runTaskCount) ++ (if (l > 0) Seq(l) else Seq.empty)) { count =>
      runTask(
        client,
        runTaskEcsClusterName,
