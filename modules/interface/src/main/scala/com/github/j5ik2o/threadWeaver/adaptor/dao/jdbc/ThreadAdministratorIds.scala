...
) extends Record with ThreadAdministratorIdsRecord

case class ThreadAdministratorIdss(tag: Tag) extends TableBase[ThreadAdministratorIdsRecordImpl](tag, "thread_administrator_ids") {
...
override def * : ProvenShape[ThreadAdministratorIdsRecordImpl] = (id,threadId,accountId,adderId,createdAt,updatedAt) <> (ThreadAdministratorIdsRecordImpl.tupled, ThreadAdministratorIdsRecordImpl.unapply)
}
