...
trait ThreadMessageComponent extends SlickDaoSupport {
  ...
  case class ThreadMessageRecordImpl(
      ...
      with ThreadMessageRecord

  case class ThreadMessages(tag: Tag)
      extends TableBase[ThreadMessageRecordImpl](tag, "thread_message")
      with SoftDeletableTableSupport[ThreadMessageRecordImpl] {
    ...
    override def * : ProvenShape[ThreadMessageRecordImpl] =
      (id, deleted, threadId, senderId, `type`, body, createdAt, updatedAt) <> (ThreadMessageRecordImpl.tupled, ThreadMessageRecordImpl.unapply)
  }
