...
case class ThreadRecordImpl(
    ...
    ) extends SoftDeletableRecord with ThreadRecord

case class Threads(tag: Tag) extends TableBase[ThreadRecordImpl](tag, "thread") with SoftDeletableTableSupport[ThreadRecordImpl] {
...
override def * : ProvenShape[ThreadRecordImpl] = (id,deleted,persistenceTag,sequenceNr,creatorId,parentId,title,remarks,createdAt,updatedAt,removedAt) <> (ThreadRecordImpl.tupled, ThreadRecordImpl.unapply)
}
